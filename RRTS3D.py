...
class RRTStar3D:
    def __init__(self, boundary, obstacles, start, goal, step_size=0.5, max_iter=2000, goal_sample_rate=0.1, neighbor_radius=2.0):
        ...
        # Force start and goal to the ground (z = zmin)
        zmin = boundary[2]
        self.start = Node([start[0], start[1], zmin])
        self.goal = Node([goal[0], goal[1], zmin])
    ...
    def sample(self):
        ...
        # Sample only on the ground: z is fixed at zmin.
    ...
    def steer(self, from_node, to_point):
        ...
        # Force new node to be on the ground.
        new_pos[2] = self.boundary[2]
    ...
    def extract_path(self):
        ...
        while node is not None:
            # Force each path point to be on the ground.
            ground_point = [node.pos[0], node.pos[1], self.boundary[2]]
            path.append(ground_point)
            node = node.parent
        return path[::-1]
...
def draw_obstacle(ax, obs, color='black', alpha=0.5):
    ...
    # Define vertices for each face of the obstacle box.
    vertices = [
...
def plot_environment(boundary, obstacles, start, goal, path, tree_nodes):
    ...
    # Draw the ground surface (black)
    draw_ground(ax, boundary)
    
    # Draw obstacles (translucent white with full height)
    ...
    ax.set_title('RRT* 3D Path Planning (Ground-Constrained)')
    ax.legend()
    plt.show()
...
def main():
    ...
    # Force start and goal to be on ground for plotting.
    zmin = boundary[2]
    start_ground = [start[0], start[1], zmin]
    goal_ground = [goal[0], goal[1], zmin]
    plot_environment(boundary, obstacles, start_ground, goal_ground, path, planner.nodes)
